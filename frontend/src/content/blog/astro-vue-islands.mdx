...
The entire article renders as static HTML. Zero JavaScript. The only interactive piece is the `ThemeToggle`, which gets its own tiny Vue runtime.
...
The theme toggle reads from `localStorage`, updates the `data-theme` attribute on `<html>`, and persists the choice. This requires client-side JavaScript — there's no way to do it with static HTML.
...
This is used with `client:load` because the theme toggle needs to be interactive immediately — users expect to click it as soon as they see it.
...
- Only the theme toggle (~2KB of Vue runtime + component) gets hydrated
