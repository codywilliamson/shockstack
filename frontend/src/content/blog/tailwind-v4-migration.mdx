...
Because the `--ss-*` variables change when the `data-theme` attribute switches (dark tokens are on `:root`, light tokens are on `[data-theme="light"]`), every Tailwind utility automatically adapts to the current theme. No dark mode variant needed.
...
ShockStack doesn't need this pattern at all. Since the design tokens change via CSS variables when the theme switches, a single class handles both modes:

```html
<div class="bg-bg-primary text-fg-primary">
```

No `dark:` prefix. The CSS variables resolve to the right values based on the active theme. This cuts the class count roughly in half on themed elements.
